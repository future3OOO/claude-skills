...
One importable owner for the envelope the advisor transport validates, so the
contract stops living inside a shell heredoc where neither the quality gate nor
a direct test can reach it. Binding proves only that the evidence names this
checkout at this HEAD - never that the graph evidence inside it is true,
complete, or machine-produced.
"""
...
def _head_sha(identity: RepoIdentity) -> str:
...
def validate_envelope(value: object, identity: RepoIdentity) -> None:
    """Return silently if the envelope is valid, or refuse naming the condition that failed.
