...
- `redFailure` names the product failure the ACT observes - an assertion marker or the actual production exception or diagnostic; failing earlier is evidence for no item, and the tdd skill's [recorder reference](../tdd/recorder.md) owns what the recorder accepts. The first RED of a new Seam asserts the Seam's existence (`assert hasattr(db, "x"), MARKER`).
