...
The map owns the behavior, Seam, expected outcome, and behavior-specific `redFailure` - an assertion marker or the actual production exception or diagnostic. Directly invoked pytest and unittest: RED is valid only when collection/loading/setup reaches at least one executed test and the marker is emitted by that test's failure, its assertion or the production exception it raised. A report block the runner entered through a fixture (`setUp`, `setUpClass`, `setUpModule`, `asyncSetUp`), a loader failure, missing APIs, imports, syntax, and zero-test runs do not open a cycle; a pytest run whose FAILURES section carries more header-shaped lines than failed tests is unattributable and refuses, naming both counts, because printed output is never the assertion. Any other command is a direct ACT: a failing run opens RED with reach recorded unresolved when its terminal failure is not an identifiable loader diagnostic (Python import or syntax, Node missing module, a missing file or command) and its output carries the declared failure; its exit 0 records GREEN through its own RED and never a baseline. Every executed mapped run is retained in the current TDD evidence, a refused one with its refusal reason and no status, cycle, or phase change.
