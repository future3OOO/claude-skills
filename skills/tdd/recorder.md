...
The map owns the behavior, Seam, expected outcome, and behavior-specific `redFailure` marker, which may name the actual production exception or diagnostic. For directly invoked pytest and unittest, RED is valid only when collection/loading/setup reaches at least one executed test and the marker is emitted by that test's failure - its assertion or the production exception it raised. Printed output is never the assertion: a pytest run whose FAILURES section carries more header-shaped lines than failed tests is unattributable and refuses, naming both counts. Missing APIs, imports, fixtures, syntax, collection/setup failures, and zero-test runs do not open a cycle. Any other exact-bound command is an ACT on the real production Interface: a failing run whose output carries the declared failure opens RED with reach recorded unresolved, an identifiable pre-Interface failure refuses with the reason retained, and its exit 0 records GREEN through its own RED but never a baseline. Every executed mapped run is retained in the current TDD evidence document, a refused one with its refusal reason and no status, cycle, or phase change.
