...
The map owns the behavior, Seam, expected outcome, and behavior-specific `redFailure` - an assertion marker or the actual production exception or diagnostic. Directly invoked pytest and unittest: RED is valid only when at least one test executed and the marker belongs to that test's terminal exception, its assertion or the production exception it raised. A fixture block (`setUp`, `setUpClass`, `setUpModule`, `asyncSetUp`), a loader failure, a terminal import, syntax or zero-test failure, a pytest FAILURES section with more header-shaped lines than failed tests (printed output is never the assertion), and a traceback style without pytest's E-prefixed report (`--tb=no`, `--tb=line`, `--tb=native`) refuse. Any other command is a direct ACT: a failing run opens RED with reach recorded unresolved when its terminal failure is not an identifiable loader diagnostic (Python import or syntax, Node missing module, a missing file or command) and its output carries the declared failure. A recovered loader traceback followed by a plain non-zero exit is indistinguishable in captured output from a fatal one with buffered output, so that shape refuses as a known limitation. Exit 0 records GREEN through its own RED and never a baseline. Every executed mapped run is retained in the current TDD evidence, a refused one with its refusal reason and no status, cycle, or phase change.

A valid RED records its item red and opens its cycle whatever the map's other items are doing; the edit hook names any contract item still without its RED instead of refusing. Mapped proof surfaces must resolve inside the repository: unittest selectors, discover start directories, and pytest targets that do not resolve under the repository root refuse at cycle-open. That promise is target-name resolution, not executed-source attestation — the ledger is continuity, and deliberately routing executed test source from outside the repository through an in-repo re-export, `load_tests`, or conftest delegation is fabricated proof in the audited deception class. A passing runner-backed RED baselines the item. GREEN must rerun the same normalized test surface, not merely the same spelling. For directly invoked stdlib unittest or pytest, fail-fast and verbosity aliases may differ; selectors, target, config, runner, behavior ID, and Seam remain load-bearing. Unknown runners remain exact-command bound.
