...
def _active_candidate(identity: RepoIdentity, value: str) -> tuple[str, str]:
    """The active slug and instance id; a workflow that cannot record a run refuses before it."""
    state = bound_state(identity, safe_slug(value))
    # commit_tdd re-checks this under the lock; refusing here keeps a closed
    # window from paying for the command first.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # Candidate identity is a pre-run fact, so an unfinished cycle refuses a
        # different candidate before the command costs anything. commit_tdd
        # revalidates this same read under the lock.
        existing = read_json(path)
