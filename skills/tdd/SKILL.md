...
TDD is complete only when every contract item is GREEN or baseline `already-satisfied`, every preservation item is GREEN, already satisfied, or omitted with evidence, every superseded item's terminal replacement is GREEN, no proof gap or reassessment remains, the broader relevant suite passes, and no behavior-changing edit occurred after the last applicable GREEN.
