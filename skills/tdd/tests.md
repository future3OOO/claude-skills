...
The RED must reach the mapped Seam and fail with the claimed product failure. Give the proving assertion a behavior-specific marker, or declare the production exception or diagnostic the ACT observes, and record it as `redFailure` in preflight. What the recorder accepts and refuses is owned by [recorder.md](recorder.md).
