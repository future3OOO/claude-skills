...
approval_helper="$script_dir/commit-approval.py"
if [[ "$phase" == "precommit-challenge" ]]; then
  python3 "$approval_helper" clear --cwd "$cwd"
fi
...
Finish with a final line exactly: Verdict: commit-ready, Verdict: fix-before-commit, or Verdict: context-mismatch."
    ;;
  *) printf 'error: unsupported phase: %s\n' "$phase" >&2; exit 2 ;;
esac
...
output_file=$(mktemp "$state_dir/output.XXXXXX")
set +e
...
if [[ ! -s "$output_file" ]]; then
  printf 'error: codex advisor returned empty stdout\n' >&2
  rm -f "$output_file"
  exit 2
fi
...
cat "$output_file"
if [[ "$phase" == "precommit-challenge" ]]; then
  python3 "$approval_helper" record --cwd "$cwd" --output "$output_file"
fi
rm -f "$output_file"
