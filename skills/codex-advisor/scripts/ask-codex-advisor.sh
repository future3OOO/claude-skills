...
sys.stdout.write((state.get("intent") or "") + "\0")')
  if [[ -n "$active_design_evidence" ]]; then
    ...
    if [[ -z "$recorded_design" ]] || ! python3 - "$script_dir/../../.." "$design_declaration_file" "$recorded_design_file" <<'PY'
import json, sys
sys.path.insert(0, sys.argv[1])
from hooks.lib.workflow_documents import same_design_declaration
with open(sys.argv[2], encoding="utf-8") as handle:
    candidate = json.load(handle)
with open(sys.argv[3], encoding="utf-8") as handle:
    recorded = json.load(handle)
raise SystemExit(0 if same_design_declaration(recorded, candidate) else 1)
PY
    then
