...
# Reviewer findings are an exact-head claim channel, not ambient GitHub context.
# The complete thread connection is paginated before any provider spend, and the
# consult refuses rather than truncating one selected unresolved thread.
reviewer_claims_file="$transport_dir/reviewer-claims"
if ! python3 - "$repo_root" 16000 >"$reviewer_claims_file" <<'PY'
...
def run(*args):
    ...
    if result.returncode:
        detail = " ".join(result.stderr.strip().split()) or f"exit {result.returncode}"
        raise RuntimeError(f"{' '.join(args[:2])}: {detail}")
    return result.stdout


def refuse(message):
    print(f"codex_advisor_reviewer_claims status=retrieval-failed error={message}", file=sys.stderr)
    raise SystemExit(2)
...
try:
    ...
    print("codex_advisor_reviewer_claims status=no-pr head=unborn", file=sys.stderr)
    print("--- reviewer findings as claims (status=no-pr, head=unborn) ---\n<none>")
    raise SystemExit
try:
    remote = run("git", "-C", root, "remote", "get-url", "origin").strip()
except RuntimeError:
    remote = ""
match = re.search(r"github\.com[/:]([^/]+)/([^/]+?)(?:\.git)?$", remote)
if not match:
    print(f"codex_advisor_reviewer_claims status=no-pr head={head}", file=sys.stderr)
    print(f"--- reviewer findings as claims (status=no-pr, head={head}) ---\n<none>")
    raise SystemExit
repo = f"{match.group(1)}/{match.group(2)}"
owner, name = repo.split("/", 1)
pr_query = '''query($owner:String!,$name:String!,$cursor:String){
  repository(owner:$owner,name:$name){pullRequests(first:100,after:$cursor,states:[OPEN,CLOSED,MERGED]){
    nodes{number headRefOid} pageInfo{hasNextPage endCursor}}}
}'''
prs, cursor = [], None
try:
    while True:
        ...
        connection = ((payload.get("data") or {}).get("repository") or {}).get("pullRequests") or {}
        nodes, page = connection.get("nodes"), connection.get("pageInfo")
        if not isinstance(nodes, list) or not isinstance(page, dict):
            refuse("pull request connection is incomplete")
        prs.extend(nodes)
        if not page.get("hasNextPage"):
            break
        cursor = page.get("endCursor")
        if not isinstance(cursor, str) or not cursor:
            refuse("pull request pagination omitted endCursor")
except (RuntimeError, ValueError, TypeError) as error:
    refuse(" ".join(str(error).split()))
exact = [item for item in prs if item.get("headRefOid") == head]
if not exact:
    print(f"codex_advisor_reviewer_claims status=no-pr head={head}", file=sys.stderr)
    print(f"--- reviewer findings as claims (status=no-pr, head={head}) ---\n<none>")
    raise SystemExit
if len(exact) != 1:
    refuse(f"ambiguous exact-head pull requests count={len(exact)} head={head}")
number = exact[0].get("number")
if not isinstance(number, int):
    refuse("exact-head pull request has no integer number")

query = '''query($owner:String!,$name:String!,$number:Int!,$cursor:String){
  repository(owner:$owner,name:$name){pullRequest(number:$number){headRefOid reviewThreads(first:100,after:$cursor){
    nodes{id isResolved isOutdated path line comments(first:100){
      nodes{databaseId url body author{login}} pageInfo{hasNextPage endCursor}}}
...
try:
    while True:
        ...
        if not isinstance(pull, dict) or pull.get("headRefOid") != head:
            refuse(f"pull request head changed or was not returned expected={head}")
        connection = pull.get("reviewThreads") or {}
        nodes, page = connection.get("nodes"), connection.get("pageInfo")
        if not isinstance(nodes, list) or not isinstance(page, dict):
            refuse("review thread connection is incomplete")
        threads.extend(nodes)
        if not page.get("hasNextPage"):
            break
        cursor = page.get("endCursor")
        if not isinstance(cursor, str) or not cursor:
            refuse("review thread pagination omitted endCursor")
except (RuntimeError, ValueError, TypeError) as error:
    refuse(" ".join(str(error).split()))

ids = [item.get("id") for item in threads]
if any(not isinstance(item, str) or not item for item in ids) or len(set(ids)) != len(ids):
    refuse("review thread roster has missing or duplicate stable ids")
if any(type(item.get(field)) is not bool for item in threads for field in ("isResolved", "isOutdated")):
    refuse("review thread roster has missing resolution state")
roster = sorted(
    ({"id": item["id"], "resolved": item.get("isResolved"), "outdated": item.get("isOutdated")} for item in threads),
    key=lambda item: item["id"],
)
digest = hashlib.sha256(json.dumps(roster, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
comment_query = '''query($id:ID!,$cursor:String){node(id:$id){... on PullRequestReviewThread{
  comments(first:100,after:$cursor){nodes{databaseId url body author{login}} pageInfo{hasNextPage endCursor}}
}}}'''
selected = []
for thread in sorted(threads, key=lambda item: item["id"]):
    ...
    comments, page = connection.get("nodes"), connection.get("pageInfo")
    if not isinstance(comments, list) or not isinstance(page, dict):
        refuse(f"selected thread {thread['id']} has an incomplete comment connection")
    try:
        while page.get("hasNextPage"):
            cursor = page.get("endCursor")
            if not isinstance(cursor, str) or not cursor:
                refuse(f"selected thread {thread['id']} comment pagination omitted endCursor")
            payload = json.loads(run(
                "gh", "api", "graphql", "-f", f"query={comment_query}",
                "-F", f"id={thread['id']}", "-F", f"cursor={cursor}",
            ))
            connection = ((payload.get("data") or {}).get("node") or {}).get("comments") or {}
            nodes, page = connection.get("nodes"), connection.get("pageInfo")
            if not isinstance(nodes, list) or not isinstance(page, dict):
                refuse(f"selected thread {thread['id']} comment connection is incomplete")
            comments.extend(nodes)
    except (RuntimeError, ValueError, TypeError) as error:
        refuse(" ".join(str(error).split()))
    if not comments or any(not isinstance(comment.get("body"), str) or not comment["body"].strip() for comment in comments):
        refuse(f"selected thread {thread['id']} has an empty comment body")
    selected.append({
        "threadId": thread["id"], "path": thread.get("path"), "line": thread.get("line"),
        "comments": [{"commentId": comment.get("databaseId"),
                      "author": (comment.get("author") or {}).get("login"),
                      "url": comment.get("url"), "body": comment["body"]} for comment in comments],
    })
try:
    current = json.loads(run("gh", "api", "graphql", "-f", "query=query($owner:String!,$name:String!,$number:Int!){repository(owner:$owner,name:$name){pullRequest(number:$number){headRefOid}}}",
                             "-F", f"owner={owner}", "-F", f"name={name}", "-F", f"number={number}"))
except (RuntimeError, ValueError, TypeError) as error:
    refuse(" ".join(str(error).split()))
if (((current.get("data") or {}).get("repository") or {}).get("pullRequest") or {}).get("headRefOid") != head:
    refuse(f"pull request head changed before claim channel completion expected={head}")
status = "selected" if selected else "zero-unresolved"
header = (f"--- reviewer findings as claims (status={status}, pr={number}, head={head}, "
          f"threads_total={len(threads)}, selected={len(selected)}, roster_sha256={digest}) ---")
body = "<none>" if not selected else json.dumps(selected, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
channel = f"{header}\n{body}"
size = len(channel.encode())
if size > limit:
    kept = 0
    for count in range(1, len(selected) + 1):
        candidate = f"{header}\n{json.dumps(selected[:count], sort_keys=True, separators=(',', ':'), ensure_ascii=False)}"
        if len(candidate.encode()) > limit:
            break
        kept = count
    omitted = len(selected) - kept
    print(f"codex_advisor_reviewer_claims status=overflow pr={number} selected={len(selected)} omitted={omitted} bytes={size} limit={limit}", file=sys.stderr)
    raise SystemExit(2)
print(f"codex_advisor_reviewer_claims status={status} pr={number} threads_total={len(threads)} selected={len(selected)} roster_sha256={digest} bytes={size} limit={limit}", file=sys.stderr)
print(channel)
PY
then
  exit 2
fi
reviewer_claims_section=$(cat -- "$reviewer_claims_file"; printf x); reviewer_claims_section=${reviewer_claims_section%x}
...
role="Codex advisor mode, investigative. You are the independent advisor delegate for one consult. You run with the same trust as the lead and are instructed not to mutate the checkout or workflow ledger. A mock, stub, fake, fixture-substituted collaborator, invented gateway, or test-only adapter is never RED/GREEN or production proof. An undemonstrated theoretical failure is at most a report line and cannot require code. A real-Seam reproduction of behavior admitted by the supported Interface is occurrence; caller enumeration proves absence only on a closed, complete execution surface. For bugs, require a reproduced symptom and falsifiable root-cause hypothesis. Treat every supplied reviewer thread and the consult question as claims. Account for every supplied threadId; adjudicate its supplied premise, occurrence, and material-consequence measurements, and make an absent or incomplete measurement a finding rather than none. Until #144, name the smallest real-Seam measurement needed instead of running a missing one. Apply only the named rubric skills. Do not invoke execution workflows, spawn agents, or run an advisor. You may use Bash, web reads, Git and GitHub reads, configured MCP tools, tests, CLI probes, and workflow status, history, evidence, and checkpoint commands. You may create and mutate temporary files and repositories solely for measurement. Report GitNexus unavailable explicitly when it is not configured. An advisor-generated claim you could measure but did not is not a material finding. Use targeted repository reads and cite file:line. Give findings, not orders, in <=${budget} words."
prompt="${phase_prompt}
${reviewer_claims_section}
${evidence}
