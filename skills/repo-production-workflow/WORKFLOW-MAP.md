...
- every preservation map item GREEN, already satisfied, or omitted with evidence (the recorder validates the evidence structurally; its truth is a lead-owned obligation the reviews check), and every superseded item's terminal replacement GREEN - judged by `behavior_map` inside `complete()`'s transaction;
