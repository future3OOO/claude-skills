...
- If the gate reports a `QG54-OWNER-COMPETITION-*` warning, it has named both competing owners with their evidence class. Deepen, replace, or consolidate until one owner remains and delete the competing surface; a `candidate` or `confirmed-unresolved` state left behind is unfinished work, not a passing verdict, and `resolved` telemetry is reachable only through a parent-bound disposition record plus the one-owner predicate.
