...
- Install estate changes into `~/.claude` as soon as they are verified locally rather than waiting for the merge, because running the change live is how its defects surface on a real pass. Scoped non-`main` installation, disposable scratch-input adaptation, and rebase after another slice merges follow the concurrent live-estate install contract in the estate repo's `README.md` ("Scoped install from a non-`main` branch"). Say plainly in the pass notes that the estate is ahead of `main`, naming each installed branch, commit, and path set, and re-sync to merged `main` once the PR lands so the estate never sits on a superseded branch state.
...
- A re-raised finding gets a fresh measurement. A disposition is never reusable across heads: re-run the premise and occurrence checks against the current code and quote the new numbers. Repetition is not evidence the finding is noise, and a cached conclusion is not a measurement.
