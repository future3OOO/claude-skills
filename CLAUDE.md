...
- Install verified estate changes live before merge per the scoped install contract in the estate repo's `README.md` ("Scoped install from a non-`main` branch"); name each installed branch, commit, and path set in the pass notes, and re-sync to merged `main` once the PR lands.
