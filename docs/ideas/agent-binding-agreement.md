...
In exchange, the process becomes a binding agreement with three pieces — markdown, one JSON file, and a handful of small scripts. Nothing hidden: no app, no dashboard, no database. You can read every piece:

- **Clauses** — markdown files stating the rules [1][9][10]. Deepen modules instead of spawning new ones; a new module must earn its interface by hiding complexity. Check the existing tests before writing more. Production proof must cross the real Interface and implementation path: use the real filesystem or runtime, an owned integration environment, a provider sandbox, or staging. A programmed stand-in may isolate a diagnostic hypothesis, but it is never RED/GREEN or production proof. If no real Seam can be driven, *say so* — an admitted proof gap beats manufactured green. One owner per rule; every other file points at the owner.
- **Ledger** — one JSON file per repo recording each claimed step, in order [4]. It lives in Claude Code's state directory, not in the repo — the checkout stays clean, and a new repo just gets a new ledger. The state machine rejects a transition whose predecessor is not ready. Most steps remain lead-recorded claims, but load-bearing steps have separate producer paths [5][6][7]: the TDD runner executes the command it records, the review recorder validates the findings artifact, and the advisor wrapper records its own raw result. The ordinary phase command cannot mark TDD or structured review complete. A step an agent can mark done without doing anything will eventually be marked done without anything being done.
- **Enforcement** — five lifecycle hooks reading the ledger [2][3][8]. Before an edit: production files are refused until the chain has caught up; test files open once preflight is ready, so RED can come first. After a production edit: implementation returns to in-progress and verification, lead review, and independent final review all reset to pending. Invalidation runs before post-edit quality feedback, so a failed gate cannot leave stale approval behind. Across compaction: the chain survives. At turn end: the agent can't stop with work incomplete; it finishes, or records an honest, named pause.

The result is one mechanical state machine, not a checklist. It enforces ordering, producer-owned transitions, reviewer dispositions, and completion readiness. The same readiness check powers both `complete` and the Stop latch, so they cannot disagree about whether the pass is finished.

One mechanism deserves its own paragraph, because it's what pins the claims to reality: when the review is recorded, the ledger snapshots a manifest — every reviewable file's content hash. It's Git's own content-addressing pointed at a different question: not "what is this object?" but "is the tree still what the reviewer saw?" The consult gate and completion both recompute and compare; a mismatch refuses and names the changed files. Borrowed straight from cryptography's integrity half — tamper-evidence, not tamper-proofing. Hashes detect change; nothing signs them, on purpose.
