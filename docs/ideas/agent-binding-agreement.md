...
The result is one mechanical state machine, not a checklist. Each step enters the ledger the way a transaction enters a database: validated, atomic, refused if it doesn't hold. It enforces ordering, producer-owned transitions, reviewer dispositions, and completion readiness. The same readiness check powers both `complete` and the Stop latch, so they cannot disagree about whether the pass is finished.

When the review is recorded, the ledger also snapshots a manifest — a content hash per reviewable file. The later gates recompute and compare; a mismatch refuses and names what changed. That pins the approvals to the tree; the ordering of steps is the state machine's job. Tamper-evidence, not tamper-proofing: hashes detect change, and nothing signs them, on purpose. The hashes are Git object ids — when the pass lands, the reviewed blobs settle into Git's own hash-linked history. The ledger validates the transaction; the merge is its inclusion in the chain.
