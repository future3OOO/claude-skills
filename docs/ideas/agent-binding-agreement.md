...
- The lead's disposition now carries its evidence - a structured document at a fixed path, same shape as the review recorder's - so the gate no longer closes on a bare flag. What is still unpersisted is the advisor's own output: the findings the lead dispositions are transcribed by the lead, not captured verbatim from the consult, so the two can drift and only the transcript shows it.
...
- The audit for produced evidence is now a state read. What remains manual is the fabrication check, whether the artifact came from the work. The day the harness surfaces tool-invocation counts natively, invocation matching becomes mechanical too; provenance stays manual.
