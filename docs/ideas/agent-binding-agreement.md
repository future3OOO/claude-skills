...
When the review is recorded, the ledger also snapshots a manifest — a content hash per reviewable file. The later gates recompute and compare; a mismatch refuses and names what changed. That pins the approvals to the tree; the ordering of steps is the state machine's job. Tamper-evidence, not tamper-proofing: hashes detect change, and nothing signs them, on purpose.
