...
- Name every executable by extension, not by shebang. Code graphs select on the name, so a runner with a Python shebang and no suffix never appears as a caller, and every blast-radius answer about your ledger is incomplete in a shape identical to a complete one. The convention that reads best in a shell is the one that blinds your map.
...
- The rival advisor sometimes serializes its findings across consults, one more discovery per paid round, instead of enumerating everything in the first pass. Its terminal verdict is also what gates completion, and that verdict does not follow the materiality flag it emits itself, so a finding it calls immaterial can hold the gate shut. Reviewer calibration is a real cost lever.
