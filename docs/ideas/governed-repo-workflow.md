...
4. **The hooks** (ours: five Claude Code lifecycle hooks). Gate before edits, invalidate after edits, preserve across context compaction, latch the turn end. This layer is the most harness-specific piece — ours only works because Claude Code exposes these lifecycle points; whatever agent you use needs an equivalent. It's also the layer that turns the other three from advice into physics.
