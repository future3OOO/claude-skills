...
A pattern for making coding agents produce deep, correct code instead of fast, shallow code. This is an idea file — copy-paste it to your own LLM agent and build it together. We built it on Claude Code; any harness with lifecycle hooks can do the same. The specific skills, models, and scripts are ours — swap in your own. The contract is what transfers.
...
4. **The hooks** (ours: five Claude Code lifecycle hooks). Gate before edits, invalidate after edits, preserve across context compaction, latch the turn end. This is the layer that turns the other three from advice into physics.
